...
public class NetDataWriter
{
    ...
    public void PutSBytesWithLength(sbyte[] data, int offset, ushort length)
    {
        ...
        if (length > 0)
        {
            ...
            var sourceBytes = MemoryMarshal.Cast<sbyte, byte>(source);
    ...
    public void Put(IPEndPoint endPoint)
    {
        ...
        var destination = _data.AsSpan(_position, addressSize);
        if (!endPoint.Address.TryWriteBytes(destination, out var written))
        {
