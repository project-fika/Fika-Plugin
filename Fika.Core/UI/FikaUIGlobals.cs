...
namespace Fika.Core.UI
{
	public static class FikaUIGlobals
	{
		...
		public static GClass3388 ShowFikaMessage(this PreloaderUI preloaderUI, string header, string message,
			ErrorScreen.EButtonType buttonType, float waitingTime, Action acceptCallback, Action endTimeCallback)
		{
			...
			PreloaderUI.Class2703 messageHandler = new()
			{
		...
		public static GClass3388 ShowFikaMessage(this ErrorScreen errorScreen, string title, string message,
			...
			ErrorScreen.EButtonType buttonType = ErrorScreen.EButtonType.OkButton, bool removeHtml = true)
		{
			...
			ErrorScreen.Class2481 errorScreenHandler = new()
			{
			...
			errorScreenHandler.context = (GClass3388)baseShow.Invoke(errorScreen, [closeManuallyCallback]);
