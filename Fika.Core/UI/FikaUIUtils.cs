...
namespace Fika.Core.UI
{
	internal static class FikaUIUtils
	{
		...
		public static GClass3353 ShowFikaMessage(this PreloaderUI preloaderUI, string header, string message,
			ErrorScreen.EButtonType buttonType, float waitingTime, Action acceptCallback, Action endTimeCallback)
		{
			...
			PreloaderUI.Class2678 messageHandler = new()
			{
		...
		public static GClass3353 ShowFikaMessage(this ErrorScreen errorScreen, string title, string message,
			...
			ErrorScreen.EButtonType buttonType = ErrorScreen.EButtonType.OkButton, bool removeHtml = true)
		{
			...
			ErrorScreen.Class2456 errorScreenHandler = new()
			{
			...
			errorScreenHandler.context = (GClass3353)baseShow.Invoke(errorScreen, [closeManuallyCallback]);
