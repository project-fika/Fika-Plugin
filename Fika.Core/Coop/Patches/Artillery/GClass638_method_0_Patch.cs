...
{
    public class GClass638_method_0_Patch : FikaPatch
    {
        protected override MethodBase GetTargetMethod()
        {
            return typeof(GClass638).GetMethod(nameof(GClass638.method_0));
        }

        [PatchPrefix]
        public static bool Prefix(GClass638 __instance, ref Task __result, CancellationToken cancellationToken)
        {
            __result = SetupServerArtilleryController(cancellationToken, __instance);
            return false;
        }

        private static async Task SetupServerArtilleryController(CancellationToken token, GClass638 instance)
        {
            ...
            if (coopGame == null)
            {
                throw new NullReferenceException("GClass638_method_0_Patch: CoopGame was null!");
            }
